...
module i2c_master (
            ...
            case (state)
                ...
                ACK_1 : begin
                    ...
                    case (pulse)
                        0 : begin 
                            scl_en <= 0;
                            if (count == 99) r_scl <= scl;
                            else r_scl <= r_scl;
                        end
                        1 : begin // clock stretching
                            scl_en <= 1;
                            scl_t <= (r_scl == 1) ? 1'b0 : 1'b0; // 1 : no clk stretch
                            sda_t <= 0; r_ack <= sda;            // 0 : yes clk stretch
                        end 
                        2 : begin
                            scl_t <= (r_scl == 1) ? 1'b1 : 1'b0;
                        end
                        3 : begin
                            scl_t <= (r_scl == 1) ? 1'b1 : 1'b0;
                        end
                ...
                ACK_2 : begin
                    sda_en <= 0;
                    scl_en <= 1; // CHANGE POSSIBLE
                    case (pulse)
                        0 : begin scl_t <= 0; sda_t <= 0; end
                        1 : begin scl_t <= 0; sda_t <= 0; end
                        2 : begin scl_t <= 1; sda_t <= 0; r_ack <= sda; end
                        3 : begin scl_t <= 1; end 
                    endcase
    ...
    assign scl = (scl_en == 1) ? (scl_t == 0) ? 1'b0 : 1'b1 : 1'bz;
    assign dout = rx_data;
endmodule
...
module i2c_slave (
    ...
    parameter IDLE = 0, READ_ADDR = 1, SEND_ACK1 = 2, SEND_DATA = 3, MASTER_ACK = 4,
        RECEIVE_DATA = 5, SEND_ACK2 = 6, WAIT_P = 7, DETECT_STOP = 8, WAIT_STRETCH = 9;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            //////////////////MAIN STATE LOGIC////////////////////////
                ...
                WAIT_STRETCH : begin
                    case (pulse)
                        0 : begin scl_en <= 1; scl_t <= 0; end
                        1 : begin scl_en <= 0; end //scl_en <= 1; scl_t <= 0; 
                        2 : begin end              //scl_en <= 0; 
                        3 : begin end
                    endcase
    ...
    assign scl = (scl_en == 1) ? (scl_t) : 1'bz;
...
module i2c_top(
    input clk, reset, newd, read, stretch,
    input [6:0] addr,
    input [7:0] din,
    output [7:0] dout,
    output busy,ack_err,
    ...
    i2c_slave SLAVE(clk, reset, stretch, sda, scl, ack_err_s, done);
